...
void *ConnectionHandler(void *socket_desc)
{
	...
	char reply[BUFSIZ], file_ext[BUFSIZ],server_response[BUFSIZ], client_request[BUFSIZ], file_name[BUFSIZ],user_name[BUFSIZ], password[BUFSIZ];
	char *data;
	int status[4] = {0,0,0,0};
	int flag ;
	while(1)
		{printf("\nWaiting For Client Authentication \n");
			int user1 = recv(socket,user_name,BUFSIZ,0);
		user_name[user1] = '\0';
		printf("<user_name password> of client ==> %s \n",user_name);
		
		if(!strcmp(user_name,"nitish 1234"))
		{
			if(!status[0])
			{
				write(socket, "hi", 2);
			...
			{
				write(socket,"Someone is Logged in Already",28);
			}
			
		}
		else if(!strcmp(user_name,"eashwin 12345"))
		{
			if(!status[1])
			{
				write(socket, "hi", 2);
		...
		else if(!strcmp(user_name,"cnlab 1254"))
		{
			if(!status[2])
			{
				write(socket, "hi", 2);
		...
		else if(!strcmp(user_name,"dbmslab 1234"))
		{
			if(!status[3])
			{
				write(socket, "hi", 2);
		...
		{
			write(socket,"Incorrect Details",17);
		}
		}
	while(1)
	{	
		
		printf("  ------------------------------------------------------------------------\n\nWaiting Command From Client %s ....\n",user_name);
		int l = recv(socket, client_request, BUFSIZ, 0);
		client_request[l]='\0';
		printf("Command Recieved from client %s is --- \n\t%s\n",user_name,client_request );
