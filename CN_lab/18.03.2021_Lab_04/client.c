...
int main(int argc , char **argv)
{
	...
	char request_msg[BUFSIZ], reply_msg[BUFSIZ], file_name[BUFSIZ], user_name[BUFSIZ], password[BUFSIZ];
	...
	while(1)
	{
		printf("Hi User Please provide the Authentication Details: \n");
		printf("\t User-Id: ");
		scanf("%s",user_name);
		printf("\t password: ");
		scanf("%s",password);
		strcpy(request_msg, user_name);
		strcat(request_msg," ");
		strcat(request_msg,password);
		write(socket_desc, request_msg, strlen(request_msg));
		int length = recv(socket_desc, reply_msg, BUFSIZ, 0);
		reply_msg[length] = '\0';
		//printf("%s\n", reply_msg);
		if(!strcmp(reply_msg,"hi"))
		break;
		else
		{
			printf("  %s\n",reply_msg);
	...
	while(1)
	{
		printf(" \n --------------------------------------------------------------------------------------\n\nEnter a Method that u need to perform:\n1 to GET\n2 to PUT\n3 to MGET\n4 to MPUT\n5 to EXIT\n");
		scanf("%d", &choice);
		switch(choice)
		{
			case 1:
				printf("Which file_name to get: ");
				scanf("%s", file_name);
				performGET(file_name,socket_desc);
				break;
			case 2:
				printf("Which file_name to put: ");
				scanf("%s", file_name);
...
void performGET(char *file_name,int socket_desc){
	...
	{
		int abortflag = 0;
		printf("File already present. Enter 1 -> Override it. Press any other key to Stop.\n");
		scanf("%d", &abortflag);
		if(abortflag!=1)
			return;
	}
