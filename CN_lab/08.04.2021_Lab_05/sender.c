...
void *read_thread(void *vargp) {
	...
	while(1) {
		if((numbytes = recv(new_fd, &data, A, 0)) == -1) perror("recv");
			...
			if(!(__builtin_popcount(data) & 1)) data >>= 1, acks[data] = true;
	...
	while(packet_ptr < (W + D)) {
		...
		while(!acks[k]) { // retransmission
			...
			if(t == 0) { // introduce an error
				...
				if((numbytes = send(new_fd, &data, P, 0)) == -1) perror("send");
				else printf("Retransmission: Packet #%d transferred. (Data: %d)\n", k, data);
			} else // pkt is lost
		...
		if(t == 0) { // introduce an error
			...
			if((numbytes = send(new_fd, &data, P, 0)) == -1) perror("send");
			else printf("Message sent: Packet #%d transferred. (Data: %d)\n", w_curr, data);
		} else // pkt is lost
