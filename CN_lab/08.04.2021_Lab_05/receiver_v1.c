	...
	while(1) {
		if((numbytes = recv(new_fd, &data, P, 0)) == -1) perror("recv");
			...
			if(!(__builtin_popcount(data) & 1)) {
				...
				seq = data & (7 << 28); 
				seq >>= 28;
				printf("Message Received: Packet #%d received.\n", seq);
				...
				if((w_curr <= W && (seq >= w_curr && seq < w_curr + W)) ||
					...
					if(!acks[seq]) {
						...
						w_packets[seq] = data;
						acks[seq] = true;
						printf("Packet #%d received and utilized.\n", seq);
					} else
						printf("Packet #%d received but not utilized.\n", seq);
				}
				// window movement
				while(acks[w_curr]) {
					acks[w_curr] = false;
				...
				if(t == 0) { // introduce an error
					...
					else printf("Acknowledgement Sent: Packet #%d ack contains an error.\n", seq);
				} else if(t >= 1 && t < 3) { // successfully acked
					if((numbytes = send(new_fd, &data, A, 0)) == -1) perror("send");
					else printf("Acknowledgement Sent: Packet #%d.\n", seq);
				} else // ack lost
					printf("Acknowledgement Lost: Packet #%d.\n", seq);
			}
	}
}
